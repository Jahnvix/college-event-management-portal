...
export async function GET() {
  ...
  const [users, events, registrations, announcements, recentAudit] =
    await Promise.all([
  ...
  return apiSuccess({
    distributions: {
      eventsByCategory: countByKey(
        events.map((event) => event.category),
      ),
      eventsByStatus: countByKey(
        events.map((event) => event.status),
      ),
      registrationsByStatus: countByKey(
    ...
    topEvents: [...events]
      .sort(
        (left, right) =>
          right.approvedRegistrationsCount +
            right.waitlistCount +
            right.bookmarkCount -
          (left.approvedRegistrationsCount +
            left.waitlistCount +
            left.bookmarkCount),
      )
      .slice(0, 5),
    totals: {
      ...
      approvalRate: registrations.length
        ? Math.round((totalApproved / registrations.length) * 100)
        : 0,
